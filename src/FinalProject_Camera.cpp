...
int main(int argc, const char *argv[]) {
  ...
  string descriptorClass = descriptorType == "SIFT" ? "DES_HOG" : "DES_BINARY";  // DES_BINARY, DES_HOG

  ofstream outputFile;
  outputFile.open("../ttcCamera.csv", std::ios_base::app);
  ...
  for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex += imgStepWidth) {
    ...
    if (bLimitKpts) {
      ...
      if (detectorType.compare("SHITOMASI") == 0) {  // there is no response info, so keep the first 50 as
    ...
    if (dataBuffer.size() > 1)  // wait until at least two images have been processed
    {
      ...
      matchDescriptors((dataBuffer.end() - 2)->keypoints, (dataBuffer.end() - 1)->keypoints,
                       ...
                       descriptorClass, matcherType, selectorType);
            ...
            prevBB->lidarPoints.size() > 0)  // only compute TTC if we have Lidar points
        {
          ...
          if (bVis) {
            ...
            cv::drawMatches((dataBuffer.end() - 2)->cameraImg, (dataBuffer.end() - 2)->keypoints,
                            ...
                            currBB->kptMatches, matchImg, cv::Scalar::all(-1), cv::Scalar::all(-1),
          ...
          if (saveOutput) {
            outputFile << count++ << "," << detectorType << "," << descriptorType << "," << ttcCamera << ","
                       << ttcLidar << std::endl;
          }
